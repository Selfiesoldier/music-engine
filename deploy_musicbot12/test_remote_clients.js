...
async function testClients() {
  ...
  for (const c of clients) {
    const cmd = `python3 /app/yt-dlp --extractor-args "youtube:player_client=${c}" -F "https://www.youtube.com/watch?v=ruEQPQX90fI"`;
    try {
      ...
      console.log(`[${c}]: ${hasAudio ? '✅ SUCCESS (Audio Formats Found!)' : '❌ FAILED'} | ${err || 'No audio'}`);
      if (hasAudio) {
        console.log(text.split('\n').filter(l => l.includes('audio only') || l.includes('251') || l.includes('140')).slice(0, 3).join('\n'));
      }
    } catch (e) {
      console.log(`[${c}]: Fetch error: ${e.message}`);
    }
  }
}
